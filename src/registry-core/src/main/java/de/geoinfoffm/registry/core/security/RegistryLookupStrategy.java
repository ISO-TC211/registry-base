...
public class RegistryLookupStrategy implements LookupStrategy
{

	public final static String DEFAULT_SELECT_CLAUSE = "select acl_object_identity.object_id_identity, "
			+ "acl_entry.ace_order,  " + "acl_object_identity.uuid as acl_id, " + "acl_object_identity.parent_object, "
			+ "acl_object_identity.entries_inheriting, " + "acl_entry.uuid as ace_id, " + "acl_entry.mask,  "
			...
			+ "left join acl_sid acli_sid on acli_sid.uuid = acl_object_identity.owner_sid "
			+ "left join acl_class on acl_class.uuid = acl_object_identity.object_id_class   "
			+ "left join acl_entry on acl_object_identity.uuid = acl_entry.acl_object_identity "
			+ "left join acl_sid on acl_entry.sid = acl_sid.uuid  " + "where ( ";

	private final static String DEFAULT_LOOKUP_KEYS_WHERE_CLAUSE = "(acl_object_identity.uuid = ?)";
	...
	private PermissionFactory permissionFactory = new DefaultPermissionFactory(RegistryPermission.class);
	 ...
	 */
	@Deprecated
	public RegistryLookupStrategy(DataSource dataSource, AclCache aclCache,
	...
	private String computeRepeatingSql(String repeatingSql, int requiredRepetitions) {
		...
		StringBuilder sqlStringBldr = new StringBuilder(startSql.length() + endSql.length() + requiredRepetitions
	 ...
	 * @param sids
	 */
	private void lookupPrimaryKeys(final Map<Serializable, Acl> acls, final Set<UUID> findNow, final List<Sid> sids) {
		Assert.notNull(acls, "ACLs are required");
		Assert.notEmpty(findNow, "Items to find now required");

		String sql = computeRepeatingSql(lookupPrimaryKeysWhereClause, findNow.size());

		Set<UUID> parentsToLookup = jdbcTemplate.query(sql, new PreparedStatementSetter() {
...
//					long id = (Long.valueOf(identifier)).longValue();
		...
		for (Acl inputAcl : acls.values()) {
			...
			Assert.isInstanceOf(UUID.class, ((AclImpl) inputAcl).getId(), "Acl.getId() must be UUID");
	...
	private AclImpl convert(Map<Serializable, Acl> inputMap, UUID currentIdentity) {
		...
		AclImpl result = new AclImpl(inputAcl.getObjectIdentity(), (UUID) inputAcl.getId(), aclAuthorizationStrategy,
	...
	private class ProcessResultSet implements ResultSetExtractor<Set<UUID>>
	{
		...
		public Set<UUID> extractData(ResultSet rs) throws SQLException {
			Set<UUID> parentIdsToLookup = new HashSet<UUID>(); // Set of parent_id Longs
...
//				long parentId = rs.getLong("parent_object");
		...
		private void convertCurrentResultIntoObject(Map<Serializable, Acl> acls, ResultSet rs) throws SQLException {
			UUID id = (UUID)rs.getObject("acl_id");
			...
			if (acl == null) {
				...
				ObjectIdentity objectIdentity = new ObjectIdentityImpl(rs.getString("class"), (UUID)rs
						.getObject("object_id_identity"));
